import requests, os, psutil, sys, jwt, pickle, binascii, time, urllib3, base64, datetime, re, socket, threading, ssl, pytz, aiohttp, traceback, signal, asyncio, random; 
...
async def bundle_packet_async(bundle_id, variant, K, V, region):
    ...
    packet = await GeneRaTePk(proto.hex(), packet_type, K, V)
    if not packet:
        return None
...
async def animation_packet(animation_id,key,iv):
    try:
        f={1:88,2:{1:{1:int(animation_id)}}}
        p=await CrEaTe_ProTo(f)
        if not p:return None
        h=p.hex()
        if not h:return None
        e=await encrypt_packet(h,key,iv)
        if not e:return None
        l=dec_to_hex(len(e)//2)
        if not l:return None
        return bytes.fromhex("051500"+"0"*(6-len(l))+l+e)
    except Exception as ex:
        print(f"❌ animation_packet error for {animation_id}: {ex}")
        return None
...
━━━━━━━━━━━━"""
                                ...
                                sub_arg = parts[2].lower() if len(parts) >= 3 else ""
                                ...
                                if bundle_name not in bundle_ids:
                                    ...
                                    variant = 2 if bundle_name in variant_rules and sub_arg in variant_rules[bundle_name] else 1
                                    
                                    try:
                                        # Function call (UPDATED)
                                        bundle_packet = await bundle_packet_async(
                                            bundle_id,
                                            variant,
                                            key, iv, region
                                        )

                                        if bundle_packet and online_writer and not online_writer.is_closing():
                                            online_writer.write(bundle_packet)
                                            await online_writer.drain()
                                            
                                            success_msg = {
                                                ("naruto", 2): "[FFD700]✨ Naruto Six Paths Activated",
                                                ("dreamspace", 2): "[00FFFF]✨ Dreamspace Variant 2 Activated"
                                            }.get((bundle_name, variant), f"[B][C][00FF00]✅ Done: {bundle_name}")
...
━━━━━━━━━━━━"""
                                ...
                                animations={"cannibal":914000003,"itachi":914050001,"naruto":914047001,"paradox":914044001,"frostfire":914042001,"midnight":914048001,"scorpio":914039001,"dreamspace":914051001,"aurora":914047002}
                                delays={"naruto":2,"itachi":3,"dreamspace":3,"scorpio":2,"cannibal":2,"frostfire":3,"aurora":3,"midnight":3,"paradox":3}
                                if anim_name not in animations:
                                    await safe_send_message(response.Data.chat_type,"❌ Invalid Animation Name",uid,chat_id,key,iv)
                                else:
                                    anim_id=animations[anim_name];delay=delays.get(anim_name,3)
                                    try:
                                        anim_packet=await animation_packet(anim_id,key,iv)
                                        if anim_packet and online_writer and not online_writer.is_closing():
                                            online_writer.write(anim_packet);await online_writer.drain()
                                            await asyncio.sleep(delay)
                                            bundle_packet=await bundle_packet_async(anim_id,1,key,iv,region)
                                            if bundle_packet and not online_writer.is_closing():
                                                online_writer.write(bundle_packet);await online_writer.drain()
                                            success_msg=f"[B][C][00FF00]✅ Animation+Bundle Sent: {anim_name}"
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/gali '):
                            ...
                            try:
                                ...
                                if len(parts) < 2:
                                    ...
                                    messages = [
                                        "{Name} তোর বোনকে চূ দি",
                                        "{Name} তোর মা কে চুঁ দা দিয়ে আমার শান্তি",
                                        "{Name} তোর মার ভো দা ফাটায়ে ফেলব",
                                        "{Name} তোর মা বলবে আহ আমাকে আর চু দ না",
                                        "{Name} তোর মাকে আমি ১৬০ বার চু দেছি",
                                        "{Name} তোর বোনকে আমিও চু দেছি আমার বন্ধুরাও চু দেছে",
                                        "{Name} খা নকীর পোলা",
                                        "{Name} MADARCHXD",
                                        "{Name} তোর বা প আমি",
                                        "{Name} নিজের লিমিট এ থাক আমার সাথে পাঙ্গা নিশ না",
                                        "{Name} তোর মাকে বিছানায় নিয়ে dash dash",
                                        "{Name}তোর বোনকে চ ুদেছি",
                                        "{Name} রে ন্ডির ছেলে",
                                        "বাংলাদেশে ১ নম্বর মা দা র চু দ {Name}",
                                        "{Name} ছোট ভাই কোথাকার",
                                        "{Name} তোর বউকে আমি চু দেছি",
                                        "{Name} আ ব্বা কেডা ভুলে গেছো"
            ]
        ...
        # Use ThreadPoolExecutor to avoid blocking the async loop
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[C][B][FF0000]╔══════════╗
[FFFFFF]✨ THE  BOT   IS RUNNING BY BBC
[FFFFFF]          ⚡  BIGBULL CHEATS  ❤️  
[FFFFFF]             
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF]BIGBULL CHEATS    
[FFD700]✨  TELEGRAM : [FFFFFF] ❤️  @bigbull8809
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </>BIGBULL CHEATS❄️  ⚡
"""
...
"""
                        ...
                        # EVO CYCLE STOP COMMAND - /sevo
                        if inPuTMsG.strip() == '/sevo':
                            if evo_cycle_task and not evo_cycle_task.done():
                                evo_cycle_running = False
                                evo_cycle_task.cancel()
                                ...
                                if not uids or not number:
                                    ...
                                    try:
                                        ...
                                        if number_int not in EMOTE_MAP:
                                            error_msg = f"[B][C][FF0000]❌ ERROR! Number must be between 1-22 only!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                        ...
                        if inPuTMsG.strip().startswith('/evo_c '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /evo_c uid1 [uid2] [uid3] [uid4] number(1-100) time(1-10000)\nExample: /evo_c 123456789 1 10\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not number or not time_val:
                                    error_msg = f"[B][C][FF0000]❌ ERROR! Invalid format! Usage: /evo_c uid1 [uid2] [uid3] [uid4] number(1-100) time(1-10000)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
...
│  └─ [FFA500]/evo [uid] [1-100]
├─ [FF6347]Fast Evo (25x)
│  └─ [FFA500]/evo_fast [uid] [1-100]
├─ [FF6347]Custom Evo (X times)
│  └─ [FFA500]/evo_c [uid] [1-100] [x]
...
│  └─ [1E90FF]/ai [question]
├─ [00CED1] This is in devlopment
│  └─ [1E90FF]/room lag (room code)
...
[00FFFA]║
[FFFF00]║ 👤 Developer    :: [FF1493]BIGBULL CHEATS 
[32CD32]║ 💻 Status        :: [32CD32]ONLINE
[1E90FF]║ 🛠 Version      :: [1E90FF]BBC V16
[00FFFA]╚═•══•════════════════•══•═╝"""
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("Initializing BIGBULL CHEATS Bot...")
    print("┌────────────────────────────────────┐")
    print("│ █████████████░░░░░░░░░░░░░░░░░░ │")
    print("└────────────────────────────────────┘")
    time.sleep(0.5)
